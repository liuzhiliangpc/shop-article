...
def run(request):
    ...
    if not isinstance(event["task_id"], str):
        ...
        r["msg"] = "检查数据类型"
        return r
    logger.info("[素材请求] [每批返回数据:{}条]".format(str(event["batch_size"])))
    data_response, remain_now_nums, retcode, msg = get_article(
        indexs="dw_ai_article",
    ...
    logger.info("[素材请求] [结果:{}]".format(str(ret)))
    return ret
