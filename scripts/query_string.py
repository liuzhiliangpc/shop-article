...
def get_batch_es_data(
    query_id: str, paras: List, indexs: str = "dw_article", max_nums: int = 10000
) -> List:
    """
    获取ES数据库数据，提供文档
    :param indexs: es 索引名
...
def validate_request(string_rule: str, business_category_list: List[str], no_rowkey_list: List[str], no_vector_id_list: List[str], size: int) -> bool:
    # 校验请求参数string_rule不能为空值
    if not string_rule:
        return False
    return True


def get_data_match_title(title_string_rule: str, query_id: str,
    indexs: str = "dw_article", download_count_lte: int = 0, business_category_list: List[str] = ["B2B, B2C"], size: int = 1000, params: Dict = {}) -> List:
    ...
    for key in default_keys:
        if key in params:  # 默认字段有输入才弹出
            other_params.pop(key)

    other_term_str = ""
    for key, value in other_params.items():
        other_term_str = other_term_str + """{"term": {"%s": "%s"}},""" % (
            key,
            value,
        )  # other = """{"term": {"industry_l2": "%s"}},""" % (industry_l2)
    es_datas = get_batch_es_data(
        indexs=indexs,
        query_id=query_id,
        paras=[
            title_string_rule,
            other_term_str,
            download_count_lte,
            business_category_list_str,
            no_rowkey_list_str,
            no_vector_id_list_str,
            size,
        ],
    )
...
def run(request):
    ...
    string_rule = request.get("string_rule", "")
    ...
    is_validated = False
    try:
        is_validated = validate_request(
            string_rule=string_rule,
            business_category_list=business_category_list,
            no_rowkey_list=no_rowkey_list,
            no_vector_id_list=no_vector_id_list,
            size=size,
        )
    ...
    title_string_rule = string_rule

    default_keys = ["index", "string_rule", "business_category_list", "size"]
    request_kwargs = copy.deepcopy(request)
    for key in default_keys:
        if key in request:  # 默认字段有输入才弹出
            request_kwargs.pop(
                key
            )  # request_kwargs = {"industry_l2": "律师服务", "industry_l1": "服务"}

    datas_from_elasticsearch = get_data_match_title(
        title_string_rule=title_string_rule,
        query_id=query_id,
        indexs=indexs,
        download_count_lte=download_count_lte,
        business_category_list=business_category_list,
        size=size,
